...
namespace FilmidMAUI.Services
{
    public class TmdbService
    {
        private const string Apikey = ""; // generate it later from tmdb website
        ...
        private HttpClient _httpClient => _httpClientFactory.CreateClient(TmdbHttpClientName);

        public async Task<IEnumerable<Result>> GetTrending()
        {
            var trendingMovies = await HttpClient.GetFromJsonAsync<Movie>($"{TmdbUrls.Trending}&api_key={ApiKey}");
        }
    }
    public static class TmdbUrls
    {
        ...
        public const string Action = "3/discover/movie?language=en-US&with-genres=28";
        ...
        public static string GetSimiar(int movieId, string type = "movie") => $"3/{type ?? "movie"}/{movieId}/similar?language=en-US";
    ...
    public class Result
    {
        ...
        public string Thumbnail => $"https://image.tmdb.org/t/p/w600_andh900_bestv2/{ThumbnailPath}";
        public string ThumbnailSmall => $"https://image.tmdb.org/t/p/w220_and_h330_face/{ThumbnailPath}";
        public string ThumbnailUrl => $"https://image.tmbdb.org/t/p/original{ThumbnailPath}";
    ...
    public class VideosWrapper
    {
        ...
        && (v.type.Equals("Teaser", StringComparison.OrdinalIgnoreCase) || v.type.Equals("Trailer"));
    }
    ...
    public class MovieDetail
    {
        ...
        public string backdoor_path { get; set; }
        ...
        public Spoken_Languages[] spoen_languages { get; set; }
